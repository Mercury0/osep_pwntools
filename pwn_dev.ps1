...
function adrecon {
    ...
    if ($domainStatus -eq $true) {
        ...
        if ($isSystem) {
            ...
            if (-not (Test-Path variable:cred)) {
                try {
                    ...
                    $changePasswordResult = net user $userName "Password123!" $domainFlag 2>$null
        ...
        $childDomainsString = $childDomains -join ', '
        ...
        if ($domainFlag -eq "/domain") {
            ...
            if ($allDomainTrusts) {
                ...
                foreach ($trustLink in $allDomainTrusts) {
                    ...
                    if ($trustLink.SourceName) {
                        $sourceName = $trustLink.SourceName.ToUpper()
                    }
                    else {
                        $sourceName = "UNKNOWN SOURCE"
                    }

                    if ($trustLink.TargetName) {
                        $targetName = $trustLink.TargetName.ToUpper()
                    }
                    else {
                        $targetName = "UNKNOWN TARGET"
                    }

                    Write-Output "`t$sourceName $trustDirectionSymbol $targetName [$trustAttribute]"
                }
                Write-Output "`t-------------------------"
            }
            else {
